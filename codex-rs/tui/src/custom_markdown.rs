...
impl<'a> MarkdownRenderer<'a> {
    ...
    fn start_tag(&mut self, tag: Tag<'_>) {
        match tag {
            ...
            Tag::Emphasis => self.push_style(parse_style(&self.styles.magenta_accent).add_modifier(Modifier::ITALIC)),
    ...
    fn start_heading(&mut self, level: HeadingLevel) {
        ...
        self.current_line.spans.push(Span::styled(marker, marker_style));
    ...
    fn start_blockquote(&mut self) {
        ...
        self.line_prefixes.push(Span::styled(">".to_string(), parse_style(&self.styles.dim_text)));
    }
    ...
    fn start_list_item(&mut self) {
        ...
        if let Some(last_index) = self.list_stack.last_mut() {
            ...
            self.current_line.spans.push(Span::styled(marker, parse_style(&self.styles.dim_text)));
        }
    ...
    fn add_text(&mut self, text: CowStr<'_>) {
        ...
        if let Some(highlighter) = &mut self.code_highlighter {
            ...
            for line in LinesWithEndings::from(&text) {
                ...
                self.current_line.spans.push(Span::styled(line.to_string(), parse_style(&self.styles.dim_text)));
            }
            return;
        }
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            self.current_line.spans.push(Span::styled(line.to_string(), style));
        }
    }
    
    fn add_inline_code(&mut self, code: CowStr<'_>) {
        let code_style = Style::default().fg(Color::White).bg(Color::Black);
        self.current_line.spans.push(Span::styled(code.to_string(), code_style));
    }
    ...
    fn add_horizontal_rule(&mut self) {
        ...
        self.current_line.spans.push(Span::styled(rule, parse_style(&self.styles.dim_text)));
        self.finish_current_line();
        self.needs_newline = true;
    }
    ...
    fn finish_current_line(&mut self) {
        ...
        if !self.line_prefixes.is_empty() {
            ...
            prefixed_line.spans.extend(self.current_line.spans.drain(..));
...
```"#;
        ...
        for line in &result.lines {
            ...
            assert!(!content.trim().starts_with("```"), 
                "Found fence marker in line: '{}'", content);
        }
        
        // Should have the code content
        let all_content: String = result.lines.iter()
