...
At any point, you may set the task’s **Status** to any valid state (e.g. open, wip, needsinput, done, cancelled) as appropriate. Use **needsinput** to request further clarification or resources before proceeding.

When you have finished working on the task file:
- If the task’s **Status** is "needsinput", stop immediately and await further instructions; do **not** run pre-commit hooks or invoke the Commit agent.
