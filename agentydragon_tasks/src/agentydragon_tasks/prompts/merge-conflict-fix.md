...
Refer to `WORKFLOW.md` for merge orchestration guidelines.

When a merge of a task branch into the integration branch fails due to conflicts, your job is to:
1. Check out the failing task branch (e.g. `agentydragon-02-auto-approve-predicates`).
2. Merge the integration branch into the task branch:
   ```bash
...
```
Merge branch 'BRANCH' into '{integration_branch}'
```
