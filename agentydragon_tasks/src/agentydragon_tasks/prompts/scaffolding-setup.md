...
4. **Project‑manager prompts**
   - Create `src/agentydragon_tasks/prompts/manager.md` containing the Project Manager agent prompt:
     ...
     - **Task orchestration**: Maintain the list of tasks, statuses, and dependencies; plan waves of work; and generate shell commands to launch work on tasks in parallel using `task.py` with `--agent` and `--tmux`.
...
**Reporting**
After each commit, print a short status message (e.g. “✅ Task stubs created”, “✅ task.py implemented”, etc.) and await confirmation before continuing
the next step.
